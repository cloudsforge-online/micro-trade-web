 ...
 * Your backtests.
 *
 * `GET /v1/backtests` — `trade/src/server.ts:382`. At most 100, newest first (`:386`), and the
 * caller's own: `ownerOf` (`trade/src/server.ts:728-734`) resolves the subject from the token.
 *
...
function Row({ run }: { run: Backtest }) {
  ...
  return (
    <tr>
      ...
      {/*
        `metrics` is null until the run completes — `trade/src/backtests.ts:222` writes the column
