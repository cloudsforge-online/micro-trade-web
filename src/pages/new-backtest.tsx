 ...
 * Queue a backtest.
 *
 * `POST /v1/backtests` — `trade/src/server.ts:471`. **202 and a status URL; the run has not
 * happened.** The handler validates, claims an idempotency key, writes a `queued` row and enqueues
 * the job after the claim commits (`trade/src/server.ts:525-536`). So this screen navigates to the
 ...
 *
 *   * **Fee, 10 bps.** `readBps(body, 'feeBps', 10)` — `trade/src/server.ts:487`.
 *   * **Slippage, 5 bps.** `readBps(body, 'slippageBps', 5)` — `trade/src/server.ts:488`.
 *   * **Seed, 0.** `readSeed` defaults to zero rather than randomising, and says why
 *     (`trade/src/server.ts:908-922`): a random default "would make an omitted seed produce a run
...
/** `trade/src/server.ts:487-488`. Restated here because the form sends them explicitly. */
