...
export interface AppRoute {
   ...
   * (`trade/src/server.ts:334`) makes no `authenticate()` call at all, and the comment above it
...
export const ROUTES: readonly AppRoute[] = [
  ...
  // address `POST /v1/backtests` puts in its `location` header (`trade/src/server.ts:470`), which
