 ...
 * outside 8–200 characters (`trade/src/server.ts:775-783`). The file header says why in one line
 ...
 * `trade/src/idempotency.ts:80-90`) and then makes three different decisions:
 *
 ...
 *     (`trade/src/idempotency.ts:151`, mapped at `trade/src/server.ts:260-262`). Returning the
 ...
 *     (`trade/src/idempotency.ts:150`, mapped at `trade/src/server.ts:263-265`). Retry shortly,
 *     with the SAME key.
 *
...
/** How the service spells the two idempotency refusals. `trade/src/server.ts:261`, `:264`. */
 ...
 * route anyway (`namespacedKey`, `trade/src/idempotency.ts:110-112`), so this prefix is for
 * humans, not for collision avoidance.
 *
 * Length is 45 characters, comfortably inside the service's 8–200 window
 * (`trade/src/server.ts:777`).
 */
 ...
 *     (`trade/src/server.ts:278-281`) and `503 rate_unavailable` (`:269-274`), both of which can
 ...
 * `bot_state` ("a stopped bot cannot be restarted", `trade/src/bots.ts:561`), a 403. None of those
